...
func (b *BeadsFS) SetEventBus(eb *ps.Bus) {
	b.eventbus = eb
}
...
func (b *BeadsFS) publishBeadReady(store bd.Storage, mountName, beadID string) {
	...
	if err != nil || issue == nil {
		b.eventbus.Publish("events", &Event{
			ID:     uuid.New().String(),
			TS:     time.Now().Unix(),
			Source: "beads/" + mountName,
			Type:   EventBeadReady,
			Data:   map[string]string{"bead_id": beadID},
		})
		return
	}
...
// HasMount returns true if the named mount exists.
func (b *BeadsFS) HasMount(name string) bool {
	b.mountsMu.RLock()
	defer b.mountsMu.RUnlock()
	_, ok := b.mounts[name]
	return ok
...
// HasBead returns true if the bead exists in the mount.
func (b *BeadsFS) HasBead(mount, beadID string) bool {
	b.mountsMu.RLock()
	m := b.mounts[mount]
	b.mountsMu.RUnlock()
	if m == nil {
	...
	issue, _ := m.store.GetIssue(b.ctx, beadID)
	return issue != nil
}

// HasBeadFile returns true if the file exists for the bead.
func (b *BeadsFS) HasBeadFile(mount, beadID, file string) bool {
	validFiles := []string{"json", "title", "description", "design", "acceptance_criteria", "notes", "status", "assignee", "comments", "blockers", "labels", "ctl"}
	for _, f := range validFiles {
		if f == file {
			return b.HasBead(mount, beadID)
		}
	}
	return false
}

// ListBeadIDs returns all bead IDs in a mount.
func (b *BeadsFS) ListBeadIDs(mount string) []string {
	b.mountsMu.RLock()
	m := b.mounts[mount]
	b.mountsMu.RUnlock()
	if m == nil {
	...
	filter := bd.IssueFilter{ExcludeStatus: []bd.Status{bd.StatusClosed}}
	issues, _ := m.store.SearchIssues(b.ctx, "", filter)
...
// ReadMtab returns the mount table.
func (b *BeadsFS) ReadMtab() ([]byte, error) {
	return b.readMtab()
}

// ReadGlobalReady returns ready beads across all mounts.
func (b *BeadsFS) ReadGlobalReady() ([]byte, error) {
	return b.readReadyAggregate()
}

// ReadGlobalDeferred returns deferred beads across all mounts.
func (b *BeadsFS) ReadGlobalDeferred() ([]byte, error) {
	return b.readDeferredAggregate()
}

// ReadMountFile reads a file from a mount.
func (b *BeadsFS) ReadMountFile(mount, file string) ([]byte, error) {
	b.mountsMu.RLock()
	m := b.mounts[mount]
	b.mountsMu.RUnlock()
	if m == nil {
	...
	return b.readFromMount(m, file)
}

// ReadBeadFile reads a file from a bead.
func (b *BeadsFS) ReadBeadFile(mount, beadID, file string) ([]byte, error) {
	b.mountsMu.RLock()
	m := b.mounts[mount]
	b.mountsMu.RUnlock()
	if m == nil {
	...
	return b.readBeadPropertyFromStore(m.store, beadID, file)
}

// WriteCtl writes to the global ctl file.
func (b *BeadsFS) WriteCtl(data string) (int, error) {
	if err := b.Write("/ctl", []byte(data), ""); err != nil {
		return 0, err
	}
	return len(data), nil
}

// WriteMountCtl writes to a mount's ctl file.
func (b *BeadsFS) WriteMountCtl(mount, data string) (int, error) {
	b.mountsMu.RLock()
	m := b.mounts[mount]
	b.mountsMu.RUnlock()
	if m == nil {
		return 0, errors.New("mount not found")
	}
	if err := b.writeToMount(m, "ctl", []byte(data), ""); err != nil {
		return 0, err
	}
	return len(data), nil
}

// WriteBeadFile writes to a bead file.
func (b *BeadsFS) WriteBeadFile(mount, beadID, file string, data []byte) (int, error) {
	path := mount + "/" + beadID + "/" + file
...
func (b *BeadsFS) Mount(name, cwd string) error {
	...
	for _, m := range b.mounts {
		if m.cwd == cwd {
			log.Printf("mount path already mounted: cwd=%s existing=%s", cwd, m.name)
	...
	for _, m := range b.mounts {
		if strings.HasPrefix(cwd, m.cwd+"/") {
			log.Printf("mount is child of existing mount: new=%s existing=%s", cwd, m.cwd)
		}
		if strings.HasPrefix(m.cwd, cwd+"/") {
			log.Printf("mount is parent of existing mount: new=%s existing=%s", cwd, m.cwd)
		}
	}
	...
	log.Printf("Mount: name=%s cwd=%s dbPath=%s", name, cwd, dbPath)
	if err := os.MkdirAll(dbPath, 0755); err != nil {
		return fmt.Errorf("failed to create db directory: %w", err)
	}
	jsonlPath := filepath.Join(cwd, ".beads", "issues.jsonl")
	log.Printf("Mount: calling bd.OpenFromConfig with ctx=%v dbPath=%s", b.ctx, dbPath)
...
func (b *BeadsFS) Write(path string, data []byte, sessionID string) error {
	...
	if len(parts) >= 2 {
		...
		b.mountsMu.RUnlock()
		if m != nil {
			log.Printf("mount write: mount=%s endpoint=%s", parts[0], strings.Join(parts[1:], "/"))
			endpoint := strings.Join(parts[1:], "/")
...
func (b *BeadsFS) writeToMount(m *MountedProject, endpoint string, data []byte, sessionID string) error {
	...
	parts := strings.Split(endpoint, "/")
	if len(parts) == 1 && parts[0] == "ctl" {
		log.Printf("ctl command: %s", string(data))
...
func (b *BeadsFS) readListFromStore(store bd.Storage, limit int) ([]byte, error) {
	...
	for i, issue := range issues {
		...
		if err != nil {
			log.Printf("failed to get blockers: issue=%s err=%v", issue.ID, err)
	...
	case "new", "create":
		...
		if !noLint && !isIdea && description != "" {
			for _, w := range lintDescription(description) {
				log.Printf("lint warning: title=%s warning=%s", title, w)
			}
		}
		...
		for _, blockerID := range blockerIDs {
			...
			if err := store.AddDependency(b.ctx, dep, actor); err != nil {
				log.Printf("failed to add blocker: bead=%s blocker=%s err=%v", newID, blockerID, err)
			}
		}
		return nil
