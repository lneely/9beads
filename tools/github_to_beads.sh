...
# Check if already imported (list is TSV: ID\tSTATUS\tBLOCKERS\tASSIGNEE\tUPDATED\tTITLE)
if grep -q "#$ISSUE" "$BEADS/$MOUNT/list" 2>/dev/null; then
...
# Get created bead ID (list is TSV, title is field 6+; grep for the issue number)
bead_id=$(awk -F'\t' -v pat="#$number" '$6 ~ pat {print $1; exit}' "$BEADS/$MOUNT/list" 2>/dev/null)
